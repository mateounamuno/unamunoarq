...
export default function PortfolioDetailsShowcaseArea({ project, prevSlug, nextSlug }: Props) {
  ...
  const heroBg = project?.showcaseHeroBg || "/assets/img/inner-project/showcase/showcase-1.jpg";
  const subtitle = project?.subtitle || "[ UI, Web Design ]";
  const title = project?.title || "Top Paddock";
  const thumbs = project?.showcaseThumbs && project.showcaseThumbs.length > 0
    ? project.showcaseThumbs
    : [
            ...
            {thumbs.slice(0, 2).map((src, i) => (
              <div key={i} className="col-xl-6 col-lg-6">
                <div className="showcase-details-thumb mb-80">
                  <Image data-speed=".8" src={src} alt="details-thumb" width={800} height={600} style={{ height: "auto" }} />
                </div>
              </div>
            ))}
            {thumbs[2] && (
              <div className="col-xl-12">
                <div className="showcase-details-thumb mb-80">
                  <Image data-speed=".8" src={thumbs[2]} alt="details-thumb" width={1200} height={800} style={{ height: "auto" }} />
                </div>
              </div>
            )}
          </div>
